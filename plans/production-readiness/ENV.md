...
| `ConnectionStrings:Redis` | `ConnectionStrings__Redis` | Shared Redis for EURES/Jobnet ranked-result caches and Gmail sync lock when running **more than one** API replica; omit for single replica (in-memory fallback) |
